...
public class Timer : MonoBehaviour
{
    ...
    [SerializeField] float currentTime = 0f;//Serialized for debugging makes it easier to see the time
    ...
    void Update()
    {
        if (start)
        {
            if (isCountDown)
            {
                // decrease time when start is active
            ...
            {
                // increase time when start is active
                currentTime += Time.deltaTime;
            }
        }
    }

    //Create subscribable event callback to check when timer is finished
